...
public partial class ViewDiagCodes : System.Web.UI.Page
{
    ILog log = log4net.LogManager.GetLogger(typeof(ViewDiagCodes));
    ...
    public DataSet bindDiagCodeDetails(int pageIndex = 1, string sortorder = "asc", string columnname = "DiagCode")
    {
        DataSet lds = null;
        int totalrecords = 0;
        ...
        using (SqlConnection gConn = new SqlConnection(ConfigurationManager.ConnectionStrings["connString_NV_Jo"].ConnectionString))
        {
            try
            {
                ...
                if (totalrecords > 0)
                {
                    gvDiagCodeDetails.DataSource = lds;
                    gvDiagCodeDetails.DataBind();
                }
                else
                {
                    gvDiagCodeDetails.DataSource = null;
                    gvDiagCodeDetails.DataBind();
                }
    ...
    protected void Page_Changed(object sender, EventArgs e)
    {
        ...
        this.bindDiagCodeDetails(pageIndex);
    }
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        {
            cnd = " where  DiagCode like '%" + txtSearch.Text + "%' or Description like '%" + txtSearch.Text + "%'";
        }
