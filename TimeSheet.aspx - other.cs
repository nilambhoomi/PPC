...
public partial class TimeSheet : System.Web.UI.Page
{
    ...
    protected void loadProcedureDetails()
    {
        ...
        List<string> _injured = _bl.getInjuredParts(_patientIEID).Distinct<string>().ToList<string>();
         //List<string> _injured1=_bl.
        string ssqlcmd = "SELECT distinct BodyPart FROM tblProceduresDetail where PatientIE_ID=" + _patientIEID;
        using (SqlConnection con1 = new SqlConnection(ConfigurationManager.ConnectionStrings["connString_NV_Jo"].ConnectionString))
        {
            SqlCommand cmd = new SqlCommand(ssqlcmd, con1);
            con1.Open();
            ...
            {
                _injured = new List<string>();
                while (reader.Read())
                {
                    _injured.Add(reader.GetString(0));
                }
            }
        }
        //    }
            ...
            foreach (string _proc in _procedures.Distinct<Procedure>(new ProcedureEqualityComparer()).Select(s => s.MCode).ToList<string>())
            {
                ...
                foreach (string _subProcedure in _proceduredetails.Distinct(new subProcedureEqualityComparer()).Select(s => s.SubProcedure).ToList<string>())
                {
                    ...
                    Procedure_ID = (_pdExecuted != null) ? _pdExecuted.Procedure_ID : Procedure_ID;
                     var id="";
                    if(_injuredbodypart=="Other")
                    {
                        var pdid= _pdBySubProcedures.Select(m=>m.ProcedureDetail_ID).FirstOrDefault();
                        id = _proceduredetails.Where(m => m.ProcedureDetail_ID == pdid).Select(m => m.MCODE).FirstOrDefault();
                    }
                    else
                    {
                         id = _proceduredetails.Where(m => m.Procedure_ID == Procedure_ID).Select(m => m.MCODE).FirstOrDefault();
                    }
                    ...
                    if (_proc != "")
                    {
                        ...
                        else if(_injuredbodypart=="Other")
                        {
                            Table Subtbl = new Table();
                            TableRow SubRow = new TableRow();
                            //TableCell SubProc = new TableCell();                           
