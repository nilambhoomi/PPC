...
public partial class TimeSheetORTHO : System.Web.UI.Page
{
            ...
            if (Request.QueryString["PId"] != null)
            {
                ...
                if (tn != null)
                {
                    ...
                    if (FUID == 0)
                    {
                        if (_ids[0] != null)
                        {
                            FUID = Convert.ToInt32(_ids[0]);
                        }
                    }
                    hfPatientFU_ID.Value = FUID.ToString();
    ...
    protected void btnFind_Click(object sender, EventArgs e)
    {
        ...
        lblDCaseType.Text = "";
        ltNew.Text = "";
        List<PatientsByDOE_Result> _patients = getPatientsByDOEORTHO(Convert.ToDateTime(ddDate.SelectedValue));
        foreach (TreeNode tn in loadPatientsToTreeNode(_patients))
        {
            tvPatients.Nodes.Add(tn);
        }
    }
    public List<PatientsByDOE_Result> getPatientsByDOEORTHO(DateTime _doe)
    {
        ...
        //}
        DataTable _dt = _dal.getDataTable("nusp_GetPatientsByDOEORTHO", param);
        foreach (DataRow _dr in _dt.Rows)
        {
    ...
    protected void btnDownloadSI_Click(object sender, EventArgs e)
    {
        ...
        //{
           // long LocationId = Convert.ToInt64((ddLocation.SelectedValue == "All") ? "0" : ddLocation.SelectedValue);
            ...
            Response.Redirect(string.Format("~/SIDownloadSheetORTHO.aspx?D={0}", date.ToString("s", CultureInfo.InvariantCulture)));
       // }
    }
