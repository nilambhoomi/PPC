...
{
    ILog log = log4net.LogManager.GetLogger(typeof(ViewGroups));
    ...
    public DataSet bindGroupsDetails(int pageIndex = 1, string sortorder = "asc", string columnname = "name")
    {
        DataSet lds = null;
        int totalrecords = 0;
        ...
        using (SqlConnection gConn = new SqlConnection(ConfigurationManager.ConnectionStrings["connString_NV_Jo"].ConnectionString))
        {
            try
            {
                SqlCommand gComm = new SqlCommand("nusp_GetGroups_paging", gConn);
                ...
                if (totalrecords > 0)
                {
                    gvDesignationDetails.DataSource = lds;
    ...
    protected void Page_Changed(object sender, EventArgs e)
    {
        ...
        this.bindGroupsDetails(pageIndex);
    }
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        {
            cnd = " where (name like '%" + txtSearch.Text + "%') or (pages like '%"+ txtSearch.Text + "%') or (locations like '%"+ txtSearch.Text +"%') ";
        }
