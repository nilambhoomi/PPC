...
public partial class PoCReport : System.Web.UI.Page
{
    ...
    {
        string query = "select tp.ProcedureDetail_ID,pm.sex,pm.LastName+', '+pm.FirstName 'Name',(select top 1 A.MA_Providers from tblFUPatient A where A.PatientIE_ID=ie.PatientIE_ID order by A.DOE DESC) 'Provider',(select top 1 ISNULL(CONVERT(VARCHAR(10),A.DOE,101),'') from tblFUPatient A where A.PatientIE_ID=ie.PatientIE_ID order by A.DOE DESC) 'DOE',ie.Compensation as 'Case',ISNULL(CONVERT(VARCHAR(10),pm.DOB,101),'') as DOB ,ISNULL(CONVERT(VARCHAR(10),ie.DOA,101),'') as DOA ,tp.MCODE,ISnull(pm.MC,'') AS MC,ISnull(tp.Sides,'') AS Sides,ISNULL(tp.Level,'') AS Level,pm.phone+','+pm.Phone2 as Phone,lc.location,pm.policy_no,ie.ClaimNumber,(Select ins.InsCo from tblInsCos ins where ins.InsCo_ID= ie.InsCo_ID) 'Insurance', case when pp.INhouseProcbit =1 then 'InHouse' Else '' End 'Inhouse'";
        query += ",stuff((SELECT ', ' + cast(a.mcode as varchar(max)) FROM tblConsider c INNER JOIN tblprocedures a on C.procedureDetailID=a.Procedure_ID and c.PatientIE_ID = tp.patientie_id FOR XML PATH('')),1,1,'')  'Consider',case when pm.Vaccinated = 1 then 'YES' when pm.Vaccinated = 0 then 'NO' else 'NO' end as Vaccinated ";
