...
public partial class Procedures : System.Web.UI.Page
{
    ...
    protected void BindMedicationList(string BodyPart = null)
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("nusp_getMedication", con);
    ...
    protected void Edit(object sender, EventArgs e)
    {
        ...
        {
            ViewState["EditMedicationID"] = row.Cells[0].Text;
            txtMedication.Text = row.Cells[3].Text;
    ...
    protected void Del(object sender, EventArgs e)
    {
        ...
        using (GridViewRow row = (GridViewRow)((LinkButton)sender).Parent.Parent)
        {
            ViewState["DeleteMedicationID"] = row.Cells[0].Text;
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["connString_NV_Jo"].ConnectionString))
            {
                SqlCommand cmd = new SqlCommand("nusp_delMedication", con);
                cmd.CommandType = CommandType.StoredProcedure;
                SqlParameter returnParameter = cmd.Parameters.Add("RetVal", SqlDbType.Int);
                returnParameter.Direction = ParameterDirection.ReturnValue;
                cmd.Parameters.AddWithValue("@Medication", Convert.ToString(ViewState["DeleteMedicationID"]));
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
                status = (int)returnParameter.Value;
            }
            ViewState["DeleteMedicationID"] = null;
    ...
    {
        ViewState["EditMedicationID"] = null;
    ...
    {
        string constr = System.Configuration.ConfigurationManager.ConnectionStrings["connString_NV_Jo"].ConnectionString;
        using (SqlConnection con = new SqlConnection(constr))
        {
            SqlCommand cmd = new SqlCommand("nusp_ModifyMedication", con);
            cmd.CommandType = CommandType.StoredProcedure;

            if (!string.IsNullOrEmpty(Convert.ToString(ViewState["EditMedicationID"])))
            { cmd.Parameters.AddWithValue("@MedicationID", Convert.ToInt32(ViewState["EditMedicationID"])); }
