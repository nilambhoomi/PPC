...
public partial class TimeSheet : System.Web.UI.Page
{
    ...
    protected void bindBodyParts(List<string> _injuredBodyParts)
    {
        ...
        {
            ddBodyPart.Items.Add(new ListItem(s, s));
        }
    }
    ...
    protected void bindProcedures(string _bodypart, string _datetype)
    {
        ...
        ddSubProcedure.Items.Clear();
        BusinessLogic _bl = new BusinessLogic();
        List<Procedure> _procedures = _bl.GetProceduresByBody(_bodypart).Where(s => s.DateType == _datetype).Distinct<Procedure>(new ProcedureEqualityComparer()).ToList<Procedure>();
        ddProcedure.Items.Clear();
        ddProcedure.Items.Add(new ListItem("Please Select", "0"));
        foreach (Procedure _procedure in _procedures)
        {
            ddProcedure.Items.Add(new ListItem(_procedure.MCode, _procedure.ProcedureId.ToString()));
        }
    }

    protected void bindSubProcedures(long _procedureId, string MCODE)
    {
        lblAlert.Text = "";
        BusinessLogic _bl = new BusinessLogic();
        ...
        List<string> _alreadyexistsprocedures = (_bl.getProcedureDetail(Convert.ToInt64(hfPatientIE_ID.Value), Convert.ToInt64(hfPatientFU_ID.Value), MCODE).Where(s => s.DateType == rblDateType.SelectedValue).Select(s => s.SubProcedure)).Distinct<string>().ToList<string>();
        ddSubProcedure.Items.Clear();
    ...
    protected void loadProcedureDetails()
    {
        ...
        {
            List<Procedure> _procedures = _bl.GetProceduresByBody(_injuredbodypart);
            ...
            foreach (string _proc in _procedures.Distinct<Procedure>(new ProcedureEqualityComparer()).Select(s => s.MCode).ToList<string>())
            {
                ...
                _tcRequestedLabel.Text = "Requested";
                ...
                _tcScheduledLabel.Text = "Scheduled";
                ...
                _tcExecutedLabel.Text = "Executed";
                _tcExecutedLabel.ID = _injuredbodypart + _proc + "ExecutedLabel";
                _trExecuted.Cells.Add(_tcExecutedLabel);

                List<ProcedureDetail> _proceduredetails = _bl.getProcedureDetail(_patientIEID, _patientFUID, _proc);
                ...
                foreach (string _subProcedure in _proceduredetails.Distinct(new subProcedureEqualityComparer()).Select(s => s.SubProcedure).ToList<string>())
                {
                    ...
                    _tcSubProcedureTitle.Text = _subProcedure;
                ...
                #region Bind Procedure Details old
                /* foreach (ProcedureDetail _proceduredetail in _proceduredetails)
                    {
                        string _subprocedure = _proceduredetail.SubProcedure;

                        //cell to display subprocedure name
                        TableCell _tcSubProcTitle = new TableCell();
                        _tcSubProcTitle.Text = _subprocedure;

                        //cell to display requested for a subprocedure
                        TableCell _tcSubProcRequested = new TableCell();

                        //data for requested
                        TextBox _hlSubProcRequested = new TextBox();
                        if (_proceduredetail.HasValue)
                            _hlSubProcRequested.Text = ((DateTime)_proceduredetail.Requested).ToString("MM/dd/yyyy");

                        _hlSubProcRequested.ID = "R_" + _proceduredetail.Procedure_ID + "_" + _patientFUID + "_" + _procedure.MCode + "_" + _subprocedure + "_" + _injuredbodypart;
                        _hlSubProcRequested.CssClass = "date bottomborder text-center";
                        _tcSubProcRequested.Controls.Add(_hlSubProcRequested);


                        //cell to display scheduled for a subprocedure
                        TableCell _tcSubProcScheduled = new TableCell();

                        //data for scheduled
                        TextBox _hlSubProcScheduled = new TextBox();
                        if (_proceduredetail.Scheduled.HasValue)
                            _hlSubProcScheduled.Text = ((DateTime)_proceduredetail.Scheduled).ToString("MM/dd/yyyy");

                        _hlSubProcScheduled.ID = "S_" + _proceduredetail.Procedure_ID + "_" + _patientFUID + "_" + _procedure.MCode + "_" + _subprocedure + "_" + _injuredbodypart;
                        _hlSubProcScheduled.CssClass = "date bottomborder text-center";
                        _tcSubProcScheduled.Controls.Add(_hlSubProcScheduled);


                        //cell to display executed for a subprocedure
                        TableCell _tcSubProcExecuted = new TableCell();

                        //data for executed
                        TextBox _hlSubProcExecuted = new TextBox();
                        if (_proceduredetail.Executed.HasValue)
                            _hlSubProcExecuted.Text = ((DateTime)_proceduredetail.Executed).ToString("MM/dd/yyyy");

                        _hlSubProcExecuted.ID = "E_" + _proceduredetail.Procedure_ID + "_" + _patientFUID + "_" + _procedure.MCode + "_" + _subprocedure + "_" + _injuredbodypart;
                        _hlSubProcExecuted.CssClass = "date bottomborder text-center";
                        _tcSubProcExecuted.Controls.Add(_hlSubProcExecuted);

                        //add subprocedures details to their rows of procedure table
                        _trTitles.Cells.Add(_tcSubProcTitle);
                        _trRequested.Cells.Add(_tcSubProcRequested);
                        _trScheduled.Cells.Add(_tcSubProcScheduled);
                        _trExecuted.Cells.Add(_tcSubProcExecuted);
                ...
                if (_trRequested.Cells.Count > 1 || _trScheduled.Cells.Count > 1 || _trExecuted.Cells.Count > 1)
                {
                    ...
                    _trProcedure.Cells.Add(_tcProcedure);
                    if (_istitletobebind)
                    {
                        _tb.Rows.Add(_trBodyPartTitle);
                        _istitletobebind = false;
                    }
                    _tb.Rows.Add(_trProcedure);
                }
    ...
    protected void ddProcedure_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddProcedure.SelectedIndex > 0)
            txtNumber.Text = (_bl.GetProceduresCount(ddProcedure.SelectedItem.Text, Convert.ToInt64(hfPatientIE_ID.Value)) + 1).ToString();
        else
            txtNumber.Text = string.Empty;

        bindSubProcedures(Convert.ToInt64(ddProcedure.SelectedValue), ddProcedure.SelectedItem.Text);
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string _mcode = ddProcedure.SelectedItem.Text + "_" + rblDateType.SelectedValue + "_" + _finalString;

            BusinessLogic _bl = new BusinessLogic();
            string count = _bl.saveProcedureDetail(
                Convert.ToInt64(ddProcedure.SelectedValue),
                Convert.ToInt64(hfPatientIE_ID.Value),
                ((string.IsNullOrEmpty(hfPatientFU_ID.Value)) ? 0 : Convert.ToInt64(hfPatientFU_ID.Value)),
                _mcode,
               ...
               "test",
                Convert.ToInt64(hfUserID.Value),
                true
                );
            if (count != "0")
            {
