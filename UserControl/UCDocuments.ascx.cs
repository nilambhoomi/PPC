...
public partial class UserControl_UCDocuments : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            DirectoryInfo rootInfo = new DirectoryInfo(Server.MapPath("~/PatientDocument/"));
            ...
            foreach (FileInfo file in directory.GetFiles(uname + "*"))
            {
                ...
                TreeNode fileNode = new TreeNode
                {
                   ...
                   // NavigateUrl = (new Uri(Server.MapPath("~/"))).MakeRelativeUri(new Uri(file.FullName)).ToString(),
                    NavigateUrl = "https://docs.google.com/viewer?url=https://www.paintrax.com/PPC/PatientDocument/" + directory.Name.Split('(')[0].Trim() + "/" + file.Name+ "&embedded=true",
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (fup.HasFiles)
            {
                ...
                {
                    string filename = Session["PatientId"] + "_" + uploadedFile.FileName;
    ...
    private void getFileList(string path, string nodeName = "")
    {
        ...
        foreach (string file in files)
        {
            ...
            dr["PreviewPath"] = "/PatientDocument/" + nodeName.Split('(')[0].Trim() + "/" + filename;
            dt.Rows.Add(dr);
    ...
    private static string getFileCount(string path)
    {
        ...
        string uname = HttpContext.Current.Session["PatientId"].ToString() + "_";
    ...
    {
        string DestFile = Server.MapPath("~/PatientDocument/") + DropDownList1.Text;
    ...
    protected void btnRename_Click(object sender, EventArgs e)
    {
        ...
        if (Directory.Exists(Server.MapPath("PatientDocument/" + txtOldFolderName.Text)) && txtNewFolderName.Text.Trim() != "")
        {
            Directory.Move(Server.MapPath("PatientDocument/" + txtOldFolderName.Text), Server.MapPath("PatientDocument/" + txtNewFolderName.Text));
        }
        DirectoryInfo rootInfo = new DirectoryInfo(Server.MapPath("~/PatientDocument/"));
