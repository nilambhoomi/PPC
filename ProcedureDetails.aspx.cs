...
public partial class ProcedureDetails : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["PId"] != null)
            {
                ...
                PatientsByDOE_Result PatientResults = (from i in PatientResult.AsEnumerable() where (i.ExamType == "IE" && i.PatientIEId == patientIEID) select i).FirstOrDefault();

                TreeNode tn = loadPatientsToTreeNode(PatientResults);
        ...
        //}
        Logger.Info(Session["uname"].ToString() + "- Visited in  ProcedureDetails for -" + Convert.ToString(Session["LastNameIE"]) + Convert.ToString(Session["FirstNameIE"]) + "-" + DateTime.Now);
    }
    ...
    protected TreeNode loadPatientsToTreeNode(PatientsByDOE_Result _patient, bool isFromQueryString = false)
    {
